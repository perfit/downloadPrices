...
func main() {
	...
	stmt, err = db.Prepare(
		"INSERT INTO prices (ticker, trans_date, open, high, low, close, volume, " +
		"adj_close) VALUES (?, ?, ?, ?, ?, ?, ?, ?) " +
		"ON DUPLICATE KEY UPDATE open=?, high=?, close=?, volume=?, adj_close=?" )
	if err != nil {
		log.Fatal(err)
	}
...
func doSymbol(symbol string, attempts int) {
	...
	for _, rec := range records[1:] {
		_, err = bind.Run(symbol, rec[0], rec[1], rec[2], rec[3], rec[4], rec[5], rec[6], rec[1], rec[2], rec[3], rec[4], rec[5], rec[6])
		if err != nil {
			trans.Rollback()
			log.Fatal(err)
		}
